...
urls = ['https://irrcac.readthedocs.io/en/latest/irrCAC.html#module-irrCAC.weights', 'https://journals.sagepub.com/doi/pdf/10.1177/001316446002000104', 'https://psycnet.apa.org/record/1980-29309-001', 'https://psycnet.apa.org/record/1972-05083-001', 'https://www.narcis.nl/publication/RecordID/oai:repository.ubn.ru.nl:2066%2F54804', 'https://bpspsychub.onlinelibrary.wiley.com/doi/abs/10.1348/000711006X126600', 'https://agreestat.com/papers/wiley_encyclopedia2008_eoct631.pdf']

class ProfileFrame(tk.Toplevel):
    """    A class representing the profile management frame in the application. This frame allows users to create, delete, and switch between different profiles.

    Attributes:
        container (tk.Tk): The parent container for this frame.
        user_input (tk.StringVar): A variable to store user input for profile names.
        profile_name_label (ttk.Label): A label to display the active profile name.
        separator_frame (ttk.Frame): A frame to separate UI components.
        change_profile_mbutton (ttk.Menubutton): A menu button to switch profiles."""

    def __init__(self, container):
        """    Initializes the ProfileFrame with the given container, sets up the UI components, and populates the profile-related information.
    ...
    Args:
        ...
        self.profile_name_label = ttk.Label(container_frame, text=self.container.dbinteraction.active_profile, font='Arial 18', image=self.container.face_icon, compound='left')
        ...
        for user in self.container.dbinteraction.profiles:
            change_profile_menu.add_radiobutton(variable=profile_selection, value=user, label=user, command=lambda: self.change_profile(profile_selection.get()))
    ...
    def ok_cmd(self, event=None):
        """    Handles the 'Ok' button command. If no new profile is created, it closes the window. Otherwise, it creates a new profile and updates the UI.

    Args:
        event (tk.Event, optional): The event that triggered this command. Defaults to None."""
    ...
    def populate_profile_label(self):
        """    Updates the profile name label with the currently active profile."""
        self.profile_name_label.configure(text=self.container.dbinteraction.active_profile)

    def populate_change_profile_menu(self):
        """    Populates the profile switch menu with the available profiles from the database."""
        change_profile_menu = tk.Menu(self.change_profile_mbutton, tearoff=False)
        self.change_profile_mbutton.configure(menu=change_profile_menu)
        profile_selection = tk.StringVar()
        for user in self.container.dbinteraction.profiles:
    ...
    def create_new_profile(self):
        """    Handles the creation of a new profile. Adds an input field for the new profile name if it doesn't already exist."""
    ...
    def change_profile(self, profile_selection):
        """    Changes the active profile to the selected profile and updates the UI accordingly.
    ...
    def delete_profile(self):
        """    Deletes the currently active profile if there are other profiles available. Shows an error message if it's the only profile."""
...
class ScrollFrame(ttk.Frame):
    """    A class representing a scrollable frame. This frame allows adding child widgets that can be scrolled vertically.

    Attributes:
        vsb (ttk.Scrollbar): The vertical scrollbar for the frame.
        canvas (tk.Canvas): The canvas widget to hold the scrollable content.
        viewPort (ttk.Frame): The frame that acts as the viewport for the canvas.
        canvas_window (int): The window ID for the canvas viewport."""
    ...
    Args:
        ...
        self.canvas = tk.Canvas(self, bd=0, highlightthickness=0, yscrollcommand=self.vsb.set, background='#ffffff')
    ...
    def onFrameConfigure(self, event):
        """    Updates the scroll region of the canvas whenever the size of the viewport frame changes.
    ...
    def onCanvasConfigure(self, event):
        """    Updates the width of the canvas window whenever the size of the canvas changes.
    ...
    def onMouseWheel(self, event):
        """    Handles the mouse wheel event for scrolling. Adjusts the view of the canvas based on the platform.
    ...
    def onEnter(self, event):
        """    Binds the mouse wheel events when the cursor enters the control.
    ...
    def onLeave(self, event):
        """    Unbinds the mouse wheel events when the cursor leaves the control.
...
class MainHelpFrame(tk.Toplevel):
    """    A class representing the main help frame in the application. This frame provides help information about the main menu and its functionalities.

    Attributes:
        container (tk.Tk): The parent container for this frame.
        notebook (ttk.Notebook): A notebook widget to hold different help tabs."""
    ...
    Args:
        ...
        general_txt = tk.Text(tab_general, foreground='black', background='white', relief='flat', font='Arial 18', highlightthickness=0, borderwidth=0)
        general_txt.insert('end', 'Das Hilfe-Symbol gibt dir auf jeder Seite Hinweise zu den Funktionalitäten der App.\n\n')
        general_txt.insert('end', 'Auf jeder Seite werden unterschiedliche Hilfsvorschläge angezeigt, je nachdem welche Elemente gerade in der App angezeigt werden.\n\n')
        general_txt.insert('end', 'Die Navigation erfolgt über die Tabs. Jeder Tab beinhaltet nähere Informationen zu den wichtigsten Elementen, die dir in der App begegnen werden.')
        general_txt.pack(padx=15, pady=30)
        analyse_txt = tk.Text(tab_analyse, foreground='black', background='white', relief='flat', font='Arial 18', highlightthickness=0, borderwidth=0)
        ...
        rate_txt = tk.Text(tab_rate, foreground='black', background='white', relief='flat', font='Arial 18', highlightthickness=0, borderwidth=0)
        rate_txt.insert('end', 'Die Bewerten-Funktion ermöglicht es dir einen Datensatz zu importieren der von dir, oder einem\nanderen SW-Nutzer, bewertet werden soll.\n\n')
        rate_txt.insert('end', 'Dabei können die Daten mit beliebigen Labeln versehen werden (Kategorisierung).\n\n')
        rate_txt.insert('end', 'Alternativ ist es möglich den Daten kontinuierliche Werte zuzuordnen, wie es beispielsweise beim Messen von\nSehstärken der Fall wäre.')
        rate_txt.pack(padx=15, pady=30)

class ScaleHelpFrame(tk.Toplevel):
    """    A class representing the help frame for scales and weights in the application. This frame provides detailed information about different scale formats and weights.
    ...
    Args:
        ...
        self.notebook.pack(expand=True, fill='both')
        scale_txt = tk.Text(tab_scale, foreground='black', background='white', relief='flat', font='Arial 18', highlightthickness=0, borderwidth=0)
        scale_txt.tag_configure('bold', font='Arial 18 bold')
        scale_txt.insert('end', 'Das Skalenformat beschreibt Eigenschaften der zu betrachtenden Daten. Man kann zwischen den folgenden Skalenformaten\nuntescheiden:\n\n')
        ...
        weights_txt = tk.Text(tab_weights, foreground='black', background='white', relief='flat', font='Arial 18', highlightthickness=0, borderwidth=0)
        ...
        weights_txt.insert('end', "Wenn man beispielsweise das ungewichtete Cohen's κ betrachtet und den Grad an Übereinstimmung messen möchte, so würden nur die Fälle als Übereinstimmung gewertet werden, bei denen beide Räter die gleiche Kategorie auswählen.\n\n")
...
class ImportHelpFrame(tk.Toplevel):
    """    A class representing the help frame for importing data in the application. This frame provides detailed information about the supported data formats for importing.
...
class PrepAnalyseHelpFrame(tk.Toplevel):
    ...
    Args:
        ...
        self.notebook.pack(expand=True, fill='both')
        rater_txt = tk.Text(tab_rater, foreground='black', background='white', relief='flat', font='Arial 18', highlightthickness=0, borderwidth=0)
        rater_txt.tag_configure('bold', font='Arial 18 bold')
        rater_txt.insert('end', 'Auswahl der Bewerter\n\n\n', 'bold')
        rater_txt.insert('end', 'Durch die Auswahl der Bewerter wird festgelegt, von welchen Bewertern die Reliabilitätsuntersuchungen vorgenommen werden sollen.\n\nFür jeden ausgewählten Intrarater wird eine eigene Intrarater-Reliabilitätsuntersuchung für den Bewerter vorgenommen.\n\nBei der Auswahl mehrerer Interrater, wird eine Interrater-Reliabilitätsuntersuchung für alle ausgewählten Bewerter erstellt.')
        rater_txt.pack(padx=15, pady=30)
        metrics_txt = tk.Text(tab_metrics, foreground='black', background='white', relief='flat', font='Arial 18', highlightthickness=0, borderwidth=0)
        ...
        metrics_txt.insert('end', 'Durch die Auswahl der Metriken kann festgelegt werden, welche Metrikwerte für die  Reliabilitätsuntersuchungen berechnet werden sollen.\n\nWeiterführende Informationen zu den Metriken, findest du unter den folgenden Links:\n\n')
...
class ResultsHelpFrame(tk.Toplevel):
    ...
    Args:
        ...
        general_txt.insert('end', 'Die Ergebnisse der Reliabilitätsuntersuchungen werden in zwei Tabs dargestellt. In einem Tab werden die Ergebnisse der Intrarater-Analyse dargestellt und im anderen Tab die Ergebnisse der Interrater-Analyse.\nVorausgesetzt, du hast im vorherigen Fenster die Auswahl getroffen, die entsprechenden Analysen vorzunehmen.\n\n')
        ...
        general_txt.insert('end', 'Die Spalte gibt an, wie viele Subjects, oder Bewertungsobjekte, es in der Reliabilitätsuntersuchung gibt.Falls ein Bewerter 10 unterschiedliche Subjects an zwei unterschiedlichen Beobachtungszeitpunkten bewertet hat, würde in der Spalte also eine 10 stehen.\n\n')
        general_txt.insert('end', '#Replicates\n', 'bold')
        general_txt.insert('end', 'Die Spalte gibt an, wie viele Replikate es gibt. Beim oberen Beispiel, in dem ein Bewerter 10 Subjects an zwei unterschiedlichen Beobachtungszeitpunkten bewertet hat, würde in der Spalte also eine 2 stehen.\n\n')
        general_txt.insert('end', '#Rater\n', 'bold')
        general_txt.insert('end', 'Bei der Interrater-Analyse wird zusätzlich in einer Spalte angegeben, wie viele Bewerter in der Analyse betrachtet worden sind.')
        general_txt.pack(padx=15, pady=30)
        interpretation_txt = tk.Text(tab_interpratation, foreground='black', background='white', relief='flat', font='Arial 18', highlightthickness=0, borderwidth=0)
        interpretation_txt.tag_configure('bold', font='Arial 18 bold')
        interpretation_txt.insert('end', 'Wie werden die Werte interpretiert?\n\n', 'bold')
        interpretation_txt.insert('end', 'In der folgenden Tabelle wurde die gängige Interpretation der Ergebnisse der Intra- und Interrater-Analysen dargestellt. Die Interpretationsmöglichkeit gilt für alle auswählbaren Metriken.\n\n')
...
class RateHelpFrame(tk.Toplevel):
    ...
    Args:
        ...
        general_txt.insert('end', 'Das Navigations-Widget links ermöglicht es schnell zwischen den Fragen hin und her zu springen. Außerdem bietet es, neben der Statusbar oben, eine Übersicht darüber, wie viele Elemente bereits bewertet worden sind.\nZusätzlich sind die linke, bzw. die rechte, Pfeiltaste mit Hotkeys belegt, um zum vorherigen, bzw. zum nächsten, Element zu wechseln.\n\n')
        general_txt.insert('end', 'Bewerten\n', 'bold')
        general_txt.insert('end', 'Um eine Bewertung vorzunehmen, kannst du die Radiobuttons ganz links drücken.Die ersten 9 Kategorien sind zusätzlich mit den Hotkeys 1-9 belegt.\nBei kontinuierlichen Daten kann das Eingabefeld ganz links zum Bewerten genutzt werden.\n\n')
